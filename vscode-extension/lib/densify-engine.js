...
;(function (root) {
  ...
  function applyFiller(text, changes) {
    for (const word of FILLER_WORDS) {
      const re = new RegExp('\\b' + esc(word) + '\\s+', 'gi');
      text = text.replace(re, (match) => {
        changes.push({ type:'filler', original:match.trim(), replacement:'(removed)',
          confidence:0.65, explanation:`"${match.trim()}" is a filler word.` });
        return '';
      });
    }
    return text;
