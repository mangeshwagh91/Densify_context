...
;(function (root) {
  ...
  //
  // NOTE: In Chrome extension context, dynamic import() from external URLs
  // (like esm.sh) is blocked by Manifest V3 CSP. We detect extension context
  // and skip the import entirely — the heuristic v2 (~85% accuracy) is
  // more than sufficient for the extension's needs.
  ...
  const _isChromeExtension = typeof chrome !== 'undefined'
  ...
  async function loadGptTokenizer() {
    ...
    // In Chrome extension, CSP blocks external imports — skip entirely
    ...
    _loading = (async () => {
      try {
        ...
        } else if (typeof window !== 'undefined' && !_isChromeExtension) {
          // Standard browser page (not extension) — try CDN
